...
discharge_choosing_patient_id, discharge_confirm = range(59,61)
antibiotic_state, remove_antibiotic_yes_no = range(61,63)

reply_keyboard = [['Get','Update']]

editable_columns_list = ['GCS', 'Ventilation', 'SPO2', 'PR', 'BP', 'INOTROPE', 'ANALGESIA', 'SEDATION', 'ANTIBIOTIC(S)', 'Other drugs', 'INSULIN infusion', 'ULCER PROPHYLAXIS', 'REMDESIVIR', 'Anticoagulation', 'METHYLPREDNISOLONE EQUI DOSE DEXA (1.5:8)', 'TOCILIZUMAB', 'STOOL' , 'FEVER', 'FEED', 'I/O', 'RTA/DRAIN', 'Hemogram', 'Coagulogram', 'SE', 'RFT', 'ABG/VBG', 'RBS', 'Special Ix', 'Date', 'IL 6', 'Ferritin', 'CRP', 'D Dimer', 'LDH', 'CxR', 'APACHE IV', 'HAS BLED', 'GFR', 'SOFA score', 'Other Scores', 'INSTRUCTIONS',]
...
def antibiotic_line(update, context):
    ...
    if update.message.text.lower()=='same':
        context.user_data[editable_columns_list[8]] = context.user_data['patient_id_row'][editable_columns_list[column_index - 1]].values[0]
    else:
        context.user_data[editable_columns_list[8]] =  context.user_data['patient_id_row'][editable_columns_list[8]].values[0] + '\n' + update.message.text
...
def edit(update, context):
    if context.user_data['conversation_type'] == 'Update':
        button_labels = [['GCS'], ['Ventilation'], ['SPO2'], ['PR'], ['BP'], ['INOTROPE'], ['ANALGESIA'], ['SEDATION'], ['ANTIBIOTIC(S)'], ['Other drugs'], ['INSULIN infusion'], ['ULCER PROPHYLAXIS'], ['REMDESIVIR'], ['Anticoagulation'], ['METHYLPREDNISOLONE EQUI DOSE DEXA (1.5:8)'], ['TOCILIZUMAB'], ['STOOL'], ['FEVER'], ['FEED'], ['I/O'], ['RTA/DRAIN'], ['Hemogram'], ['Coagulogram'], ['SE'], ['RFT'], ['ABG/VBG'], ['RBS'], ['Special Ix'], ['Date'], ['IL 6'], ['Ferritin'], ['CRP'], ['D Dimer'], ['LDH'], ['CxR'], ['APACHE IV'], ['HAS BLED'], ['GFR'], ['SOFA score'],['Other Scores'], ['INSTRUCTIONS']]
